...
| **Token Counting API integration** (Anthropic's pre-flight `count_tokens` endpoint) for `core/harness/`'s budget enforcement | Identified as the concrete mechanism Card 03 §9 (Context Budget Management) and the Agent Contract's `token_budget` field (§8) need to actually be enforceable, rather than aspirational — not needed until `core/harness/` is being built | Phase 1 (Core Infrastructure), when `core/harness/` implements context budget enforcement |
| **Headroom-style context compression layer** (or equivalent — compresses tool outputs/logs/RAG chunks before they reach the LLM, reversible) for `core/harness/` and tool-output handling | A concrete implementation candidate for Card 02 §3/§9's tool-output data-minimization discipline and Card 03 §9's budget enforcement — evaluated, not yet adopted; any adoption must verify `source_type` tagging (Card 03 §12) survives compression before integration | Phase 1 (Core Infrastructure), alongside Token Counting integration |
