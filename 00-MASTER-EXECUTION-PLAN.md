...
| **Memory Tool** (Anthropic's first-party file-based memory directory, `/memories`, persisting across sessions) | A candidate implementation substrate for Card 03 §5's memory types — needs evaluation against the already-specified vector-store + PostgreSQL approach before `core/memory/` is built, not after | Phase 1, when `core/memory/` is being designed — decide build-vs-use-native before any memory storage code is written | Pending |
...
| **Subagent / fresh-context delegation as an explicit Card 01 §6 or Card 03 §9 principle** | Currently implicit in the multi-agent Coordinator pattern already specified — worth promoting to an explicit stated principle ("delegate large reads to a sub-agent specifically to protect the parent's context budget") rather than leaving it as an unstated convention | Phase 1-2, when the first Coordinator/sub-agent split is actually implemented — confirm the principle gets written down at that point, not just practiced | Pending |
