...
**Exit criteria for Phase 0:** All 12 files `APPROVED`. AGENTS.md in particular must be scrutinized line by line since every later file inherits from it. The agent contract template (0.12) must be approved before any agent in Phase 1+ is specced — every agent built from this point forward gets instantiated against this template, not designed ad hoc.
...
**Goal:** Wrap the Phase 1 factory in the production-grade safety net before any real data touches it. Card 06 (Security) must formalize the 4-tier risk classification (Tier 1 Read-Only → Tier 2 Reversible Write → Tier 3 External Communication → Tier 4 High-Risk Actions) with explicit approval requirements mapped to each tier — this classification feeds directly into the Policy Server (2.3/2.4) and the agent contract template's HITL requirements field.
