...
**Card Version:** 5.0
**Changelog (v5.0):** §5 Alternative Architecture Considered note — replaced stale "binary Active/Deprecated lifecycle" claim with the actual 9-stage lifecycle this section defines, matching ADR-006's Patch 3 correction (which fixed the same claim in ADR-006 itself but never propagated it back to this card, its source). Found in Working Principle #8 pre-tag sweep.
**Changelog (v4.0):** §3 heading corrected from "Six" to "Seven" Tool Design Principles — the section already enumerated seven principles; the heading was incorrect. Patch 2, Stage 10 pre-freeze.
